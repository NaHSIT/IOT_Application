...
- 📊 **动态图标与主题**：内置几十种 FontAwesome 物联网常用图标与多套主题色，支持可视化点选，轻松定制专属大盘。
...
- 🎨 **统一设计体系**：自建轻量公共样式层 `css/common.css`（设计变量 + 组件类 + 工具类），全部页面共享同一套配色、字阶与组件形态；已移除 Tailwind 运行时 CDN，弱网/离线不再裸奔。
- 🌓 **深色模式**：全部页面跟随系统深浅色自动切换，图表配色同步适配主题变量。
- 📱 **深度原生交互适配**：全面移除浏览器原生 alert/confirm 阻塞弹窗，采用定制动画的自定义 Modal 弹窗及 Toast 提示，并适配了 Android 状态栏与底部安全区 (Safe-Area)，沉浸式体验。
- 📹 **实时视频流监控**：内置独立视频监控页面，支持自定义视频流地址（如 MJPEG/HTTP 流），可在设置页配置视频流 URL 和页面标题，底部导航栏一键直达。
...
### 4. 配置视频流监控（可选）🆕
1. 在设置页的 **"视频流监控地址"** 栏填写您的视频流 URL，例如：http://192.168.1.100:8080/video。
2. 在 **"视频页面标题"** 栏自定义页面顶部显示的标题（默认为"实时监控"）。
3. 保存后点击底部导航栏的 **"视频"** 图标即可查看实时画面。
4. 视频流支持标准 MJPEG HTTP 流，请确保设备与手机处于同一局域网或视频地址可公网访问。
